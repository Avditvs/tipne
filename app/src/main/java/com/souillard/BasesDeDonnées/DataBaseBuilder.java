...
///////////Fonction de build de la dbAbreviations//////////////////////

    private void buildDbAbreviations(AbreviationsDAO dbAbreviations) {
        String[] AbreviationsList = getListsAbreviations();
        int idList = 1;
        for (String AbreviationsListsNames : AbreviationsList) {
            String[] aList = getAbrevList(AbreviationsListsNames);
            int idInList = 1;
            for (String linkedAbreviations : aList) {
                String[] separatedInformations = separateInformations(linkedAbreviations);
                Abreviations anAbreviation = setAbreviations(separatedInformations, idList, idInList);
                insertAbreviationsInDB(anAbreviation, dbAbreviations);
                idInList++;
            }
            idList++;
        }
...
/////////////////////Fonctions utiles au build de dbListes////////////////
    ...
    private void insertListeInDB(Listes aListe, ListesDAO dbListes) {
        dbListes.insertListe(aListe);
...
///////////////////Fonctions utiles au build de dbMots//////////////////////
    ...
    private void insertVerbInDB(Verbes aVerb, VerbesDAO dbVerbes) {
        dbVerbes.insertVerb(aVerb);
    }
    ...
    private void insertModelInDB(ModelsDAO dbModels, Models aModel) {
        dbModels.insertModel(aModel);
    }


    /////////////////Fonctions utiles au build de dbAbreviations ///////////////////////////////

    private String[] getListsAbreviations() {
        String[] listsAbreviations;
        listsAbreviations = context.getResources().getStringArray(R.array.ListesAbreviations);
        return listsAbreviations;
    }

    private String[] getAbrevList(String nameList) { //Récupère les abréviations des listes
        int listID = context.getResources().getIdentifier(nameList, "array", context.getPackageName());
        String[] AbrevList = context.getResources().getStringArray(listID);
        return AbrevList;
    }

    private String[] separateInformations(String linkedAbreviations) { //sépare l'abréviation de sa signification
        String[] separatedInformations = linkedAbreviations.split("/");
        return separatedInformations;
    }

    private Abreviations setAbreviations(String[] separatedInformations, int idList, int idInList) { //Permet de définir l'abréviation dans la DB avec ses ID
        Abreviations anAbreviation = new Abreviations(separatedInformations[0], separatedInformations[1], 0, 0, idList, idInList);
        return anAbreviation;
    }

    private void insertAbreviationsInDB(Abreviations anAbreviation, AbreviationsDAO dbAbreviations) {
        dbAbreviations.insertAbreviations(anAbreviation);
    }
